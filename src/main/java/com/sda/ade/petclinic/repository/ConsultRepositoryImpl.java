...
public class ConsultRepositoryImpl extends BaseRepositoryImpl<Consult, Long> implements ConsultRepository {
    ...
    @Override
    public List<Consult> findAllWithUnvaccinatedPet() {
        try {
    ...
    public List<Consult> findAllByVetIdAndDateBetween(Long vetId, Date startDate, Date endDate) {
        try{
            Session session= SessionManager.getSessionFactory().openSession();
           Query<Consult> query= session.createQuery("FROM Consult c WHERE c.veterinarian.id=:vetId AND c.date BETWEEN :startDate AND :endDate",Consult.class);
